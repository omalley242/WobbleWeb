...
function App() {
  ...
  const resetDatabase = () => {
    return fetch(`http://${window.location.host}/clear`).then(window.location.reload());
  }
  ...
  return (
    <div style={{display: 'flex', width: '100%', height: '100%', justifyContent: 'center', alignContent: 'center', padding: '0', margin: '0'}}>
      ...
      <div style={{display: 'inline-block', padding: '3%'}}>
        ...
        <br/>
        <input style={{marginTop: '10vh', height: '5%', width: '100%'}}type='button' id='ClearButton' Class='Button' onClick={resetDatabase} value='Reset Database'/>
      </div>
