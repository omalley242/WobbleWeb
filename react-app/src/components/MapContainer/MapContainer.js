...
const MapContainer = ({nodeData, pathData}) => {
    ...
    const TextBox = pathData.map((item) => {
        return <div style={{minWidth: '100%',backgroundColor: '#777', border: '1px solid #333', fontSize: '0.5em'}}>
                    <div style={{backgroundColor: '#666', height: '20%'}}>
                        {"--- Path ---"}
                    </div>
    ...
    return (
        <div className='Map-Container' style={{display: 'flex', position: 'relative', flexWrap: 'wrap', border: '3px solid #222', margin: '5vh', backgroundColor: '#444', maxWidth: '80%'}}>
            ...
            <span style={{display: 'flex', overflowY: 'scroll', overflowX: 'hidden'}}>
                {TextBox}
            </span>
        </div>
