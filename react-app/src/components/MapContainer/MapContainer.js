...
const MapContainer = ({nodeData, pathData}) => {
    ...
    const setEndNode = () => {
        fetch(`http://${window.location.host}/setEndNode?nodeId=${this.id}`);
    }
    ...
    const MapItems = nodeData.map((item, index) => (
        <div key={index} id={item.Id} className='Map-Item' style={{height: '1vw', width: '1vw', position: 'absolute', left: `${item.XCoordinate / 3.6}%`, bottom: `${item.YCoordinate / 2.4}%`}} onClick={setEndNode}>
            <svg class='node' viewBox="0 0 24 24" fill={item.Colour} xmlns="http://www.w3.org/2000/svg">
