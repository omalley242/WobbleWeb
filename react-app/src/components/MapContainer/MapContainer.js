...
const MapContainer = ({nodeData, pathData}) => {
    ...
    const MapItems = nodeData.map((item, index) => (
        <div key={index} className='Map-Item' style={{position: 'absolute', left: `${item.XCoordinate / 3.6}%`, bottom: `${item.YCoordinate / 2.4}%`}}>
            <svg transform='translate(-5, -5)' class='node' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div id={item.Id} style={{height: 1, width: 1}}>
            </div>
        </div>
    ));
    ...
    return (
        <div className='Map-Container' style={{display: 'flex', position: 'relative', height: '53.3vw', width: '80vw', flexWrap: 'wrap', border: '3px solid #222', margin: '5vh'}}>
            <Xwrapper>
                {PathItems}
                {MapItems}
            </Xwrapper>
