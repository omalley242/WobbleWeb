...
const MapContainer = ({nodeData, pathData}) => {
    ...
    const MapItems = nodeData.map((item, index) => (
        <div key={index} id={item.Id} className='Map-Item' style={{height: '1vw', width: '1vw', position: 'absolute', left: `${item.XCoordinate / 3.6}%`, bottom: `${item.YCoordinate / 2.4}%`}} onClick={setEndNode(id)}>
